...
- [x] `src/services/api/admin.ts`: Refactorizar `getAdminCarreras`, `createCarrera`, `updateCarrera` y `deleteCarrera`.
- [x] `src/app/admin/dashboard/catalogo/page.tsx`: Asegurar integración con los nuevos tipos y manejo de errores.
...
- [x] **Escenario 1: Listado de carreras** -> Al cargar la página, se deben mostrar las carreras reales del campus del administrador.
- [x] **Escenario 2: Creación exitosa** -> Al crear una carrera, se debe persistir en el backend y actualizar la lista.
- [x] **Escenario 3: Actualización exitosa** -> Al editar una carrera (nombre o estado activo), se debe reflejar en el backend.
- [x] **Escenario 4: Eliminación suave** -> Al eliminar una carrera, se debe realizar un soft delete y desaparecer de la lista activa.
- [x] **Edge case: Error de validación** -> Si el backend devuelve 400 (ej. nombre duplicado), mostrar toast descriptivo.
